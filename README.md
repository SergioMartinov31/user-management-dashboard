...
* После запуска приложение будет доступно в браузере по адресу, указанному в консоли (обычно http://localhost:5173).

* API сервер запускается на указанном порту (по умолчанию http://localhost:3001).
